...
square = coroutine.create(function()
	while true do
		love.graphics.rectangle("fill",xsquare,ysquare,20,20)
		if (xsquare == 0 and ysquare < 600) then
			ysquare=ysquare+20
		elseif (xsquare <1200 and ysquare == 600) then
			xsquare=xsquare+20
		elseif (xsquare == 1200 and ysquare < 600) then
			ysquare=ysquare+20
		elseif (xsquare < 1200 and ysquare == 0) then
			xsquare=xsquare-20
		else
			break
		end	
...
function love.load(dt)
	...
	vivo = true
	player = {x = 70, y = alturaJanela/2, speed = 200, placar = 0}
	enemies = {}
	for i=1, 10 do
		enemies[i]={}
...
function love.update(dt)
	...
	if love.keyboard.isDown('up','w') then
		if player.y > 70 then  --não passa do topo da tela
			player.y = player.y - (player.speed*dt)
		end
	elseif love.keyboard.isDown('down','s') then
		if player.y < (530) then -- não passa da base da tela
			player.y = player.y + (player.speed*dt)
		end
	end
	--Fim_Movimentação 
...
function love.draw()
	...
	love.graphics.rectangle("fill",20,20,1160,560)
	--love.graphics.rectangle(mode, x, y, width, height)
	love.graphics.setColor(0,0,0)
	love.graphics.rectangle("fill",xsquare,ysquare,20,20)
	--coroutine.resume(square)
